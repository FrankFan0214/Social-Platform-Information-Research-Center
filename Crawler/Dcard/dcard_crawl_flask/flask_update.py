...
props = {
        # Kafka集群在那裡?
        'bootstrap.servers': '<IP>',  # <-- 置換成要連接的Kafka集群
...
def update():
    ...
    select_url = """select Url 
                    ...
                    """ #如果被擋，想要重爬，可以設where條件，依照confirm_num最少的開始爬(先保留)
    ...
    for row in result:
            ...
            if i > 1:
                while i >= 1:
                    ...
                    try:
                        ...
                        message_no[mes_no] = {'user': mes_writer, 'comment': mes_content, 'time': mes_time}
        ...
        current_date = datetime.now().date()
        update_last_update = """UPDATE article_confirm
                                SET last_update = %s,
                                    confirm_num = confirm_num + 1
                                WHERE Url = %s"""
        cursor.execute(update_last_update, (current_date,url))
        connection.commit()
